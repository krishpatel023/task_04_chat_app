...
export default function TextPoll({dataBase, role, senderData, isAdmin, isVotable,user,handleAfterSubmission}) {
    ...
    <div className='w-full flex justify-center my-2'>
      {
        ...
        <div className='w-[95%] flex justify-end'>
          <div className='w-[30rem] min-h-12 rounded-3xl bg-accent-color text-accent-complementary flex flex-col px-4 py-2 gap-4'>
            {/* POLL LOGIC */}
            ...
            <span className='font-semibold text-2xl'>{dataBase?.description}</span>
            {
              dataBase?.options?.map((data,i) =>
                <button className={`w-full h-12   font-medium rounded flex justify-start gap-6 items-center text-xl border-4 border-primary-color`}>
        ...
        <div className='w-[95%] flex justify-start gap-4'>
          ...
          <div className='bg-primary-color text-primary-complementary w-[30rem] min-h-12 rounded-3xl flex flex-col px-4 py-2 gap-4 z-10'>
            {/* POLL LOGIC */}
            <h1 className='font-semibold text-4xl'>{dataBase?.title}</h1>
            <span className='font-semibold text-2xl'>{dataBase?.description}</span>
            {
              dataBase?.options?.map((data,i) =>
                <button className={`w-full h-12   font-medium rounded flex justify-start gap-6 items-center text-xl ${setColor(data.id)}`} onClick={()=>handleOptionClick(data.id)} key={i}>
                  <span className='ml-2'>{i+1}.</span>
                  <span>{data.data}</span>
                </button>
              )
